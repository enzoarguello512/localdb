...
const localDB = function (filePath = null) {
  ...
  let file =
    filePath === null ? path.join(__dirname, "/localdb.json") : filePath;
  ...
  crud.set = async (newData) => {
    return new Promise((resolve, reject) => {
      if (localDbChunk.errorCheck(newData).status)
        return reject(localDbChunk.errorCheck.message);
      ...
      crud
        ...
        .then((data) => {
          if (data.length == 0 || data.length == 1 || data.length == 2) {
            ...
            reject(
              "LocalDB don't have a valid Json data type, remove all invalid data first."
            );
          } else {
  ...
  crud.insert = async (find, newData) => {
    return new Promise((resolve, reject) => {
      if (localDbChunk.errorCheck(find).status)
        return localDbChunk.errorCheck.message;
      if (localDbChunk.errorCheck(newData).status)
        return localDbChunk.errorCheck.message;
        ...
        .then((data) => {
          ...
          getAllData.filter(function (getItem) {
            if (getItem[itemKey[0]] === undefined)
              notFound.push("Not found in LocalDB...");
          ...
          if (notFound.length != 0 && match.length === 0)
            return console.error(notFound[0]);
          if (match.length != 0) {
  ...
  crud.remove = async (item) => {
    return new Promise((resolve, reject) => {
      if (localDbChunk.errorCheck(item).status)
        return reject(localDbChunk.errorCheck.message);
   ...
   *
   * @param {object} find - Identify the object
   * @param {object} newData - New values to update
   * @param {Array.<String>} allowedFields - List of properties enabled to change value
   *
   * @returns void
   */
  crud.update = async (item, newData, allowedFields) => {
    return new Promise((resolve, reject) => {
      if (localDbChunk.errorCheck(item).status)
        return reject(localDbChunk.errorCheck.message);
      if (localDbChunk.errorCheck(newData).status)
              ...
              if (getItem[itemKey[0]] == item[itemKey[0]]) {
                ...
                if (allowedFields) {
                  allowedFields.forEach((field) => {
                    if (field in newData) getItem[field] = newData[field];
                  });
                } else getItem = newData;
              }
            }
            dataMap.push(getItem);
          });
